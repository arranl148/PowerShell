...
Version 0.6.1 - Altered temp folder
#>
...
$TempFolder = $env:TEMP
$tempPath = Test-Path -Path "$TempFolder"
If ($tempPath -eq $false)
    {
        Write-Host -ForegroundColor Yellow "$TempFolder does not appear on your device. Please create it"
        $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
    }
    else {
        Write-Host -ForegroundColor Green "Using $TempFolder as a scratch folder"
        }

$WindowsUpdateLog = "$TempFolder\Windowsupdate.log"
